...
Elution is used when there is an antibody on red cells, and you want to remove it either to get rid of it (so you can test the red cells without their interference), or because you want to test the antibodies (confirm their presence or specificity)
...
-   'Wash red cells thoroughly (e.g. by gently inverting) to remove unbound antibodies and other unbound protein (things just sitting on the red cells, not strongly bound), i.e. things which might cause cell non-specific clumping. Washing is typically done 3-4 times using a saline solution.
-   Collect the fluid from the "**last wash**" of the red cells. The idea is that this last wash should not contain any gunk capable of causing cells to agglutinate (everything should've washed away in previous washes). We will use the last wash as a negative control later.
...
-   Elution is not always necessary, e.g. if the antibody is already known, or if the antibody is not causing any problems,
